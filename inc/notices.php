...
function educore_notice_tab() {
    ...
    $current_type = isset( $_GET['type'] ) ? sanitize_key( $_GET['type'] ) : 'notice';
    ...
    <style>
        ...
        .dpt-nav-tab-item:hover .dashicons { color: #006a4e; }
            ...
            <div>
                <?php if ( current_user_can( 'manage_options' ) ) : ?>
                    ...
                    <?php elseif ( $current_type === 'gallery' ) : ?>
                        <a href="<?php echo esc_url( admin_url( 'admin.php?page=school_management_system&tab=notices&type=gallery&sub=add' ) ); ?>" class="dpt-btn-action-add">
    ...
    if ( $is_event_mode ) {
        $records = $wpdb->get_results( 
            "SELECT * FROM {$table_notices} 
             WHERE LOWER(notice_type) IN ('event', 'events') 
                OR LOWER(item_type) IN ('event', 'events') 
             ORDER BY id DESC" 
        );
    } else {
        $records = $wpdb->get_results( 
            "SELECT * FROM {$table_notices} 
             WHERE LOWER(notice_type) NOT IN ('event', 'events') 
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if ( ! empty( $row->notice_type ) && $row->notice_type !== 'Notice' && $row->notice_type !== 'Event' ) : ?>
                                        <small style="color: #006a4e; font-weight: 700;">[<?php echo esc_html( $row->notice_type ); ?>]</small>
                                    <?php endif; ?>
...
function educore_notice_events_add_edit_view( $type = 'notice' ) {
    ...
    if ( isset( $_POST['educore_save_item'] ) && wp_verify_nonce( $_POST['educore_item_nonce'], 'save_item_action' ) ) {
        ...
        $form_type        = ( $type === 'events' || $type === 'event' ) ? 'Event' : sanitize_text_field( wp_unslash( $_POST['notice_type'] ) );
        $title            = sanitize_text_field( wp_unslash( $_POST['title'] ) );
        $priority_val     = sanitize_text_field( wp_unslash( $_POST['priority'] ) );
        ...
        $description_body = wp_kses_post( wp_unslash( $_POST['description'] ) );
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="dpt-grid-row dpt-cols-3">
                    ...
                    <div class="dpt-field-node">
                        ...
                        <input type="date" name="event_date" class="dpt-input-control" value="<?php echo $item ? esc_attr( ! empty( $item->event_date ) && $item->event_date !== '1970-01-01' ? $item->event_date : ( ! empty( $item->publish_date ) ? $item->publish_date : '' ) ) : current_time( 'Y-m-d' ); ?>">
                    </div>
                </div>

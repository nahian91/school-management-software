...
function educore_staff_add_edit_view() {
    ...
    // Safely check and generate Auto Unique Staff ID if new record (e.g. GIDT-0001)
    if ( ! $is_edit || ! isset( $staff->staff_id ) || empty( $staff->staff_id ) ) {
        $max_id = $wpdb->get_var( "SELECT MAX(id) FROM {$table_staff}" );
        $next_num = intval( $max_id ) + 1;
        $generated_staff_id = 'GIDT-' . str_pad( $next_num, 4, '0', STR_PAD_LEFT );
        
        // Double check uniqueness
        while ( $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$table_staff} WHERE staff_id = %s", $generated_staff_id ) ) ) {
            $next_num++;
            $generated_staff_id = 'GIDT-' . str_pad( $next_num, 4, '0', STR_PAD_LEFT );
        }
    } else {
    ...
    if ( isset( $_POST['educore_save_staff'] ) && isset( $_POST['educore_staff_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['educore_staff_nonce'] ) ), 'save_staff_action' ) ) {
        ...
        $exists_query = $wpdb->prepare( "SELECT id FROM {$table_staff} WHERE staff_id = %s", $posted_staff_id );
        if ( $is_edit ) {
            $exists_query .= $wpdb->prepare( " AND id != %d", $staff_id );
        }
        if ( $wpdb->get_var( $exists_query ) ) {
            $db_error = __( 'Error: This Staff ID is already registered. Please use a unique identifier.', 'educore' );
        } else {
            // Handle File Upload
    ...
    <div class="bg-white p-4 rounded shadow-sm border">
        ...
        <form method="POST" action="" enctype="multipart/form-data" id="educoreStaffForm" novalidate>
            ...
            <div class="educore-step-content active" id="educore-step-1">
                ...
                <div class="row">
                    <div class="col-md-3 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Staff / Employee ID', 'educore' ); ?> <span class="text-danger">*</span></label>
                        <input type="text" name="staff_id" class="form-control text-uppercase fw-bold text-success bg-light" value="<?php echo esc_attr( $generated_staff_id ); ?>" required>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Order Number', 'educore' ); ?></label>
                        <input type="number" name="order_number" class="form-control" placeholder="e.g., 1" value="<?php echo ( $staff && isset( $staff->order_number ) ) ? esc_attr( $staff->order_number ) : '0'; ?>" min="0">
                    </div>
                    <div class="col-md-3 mb-3">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( "Father's Name", 'educore' ); ?></label>
                    ...
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( "Mother's Name", 'educore' ); ?></label>
                ...
                <div class="row">
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="date" name="dob" class="form-control" value="<?php echo ( $staff && isset( $staff->dob ) ) ? esc_attr( $staff->dob ) : ''; ?>" required>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Gender', 'educore' ); ?> <span class="text-danger">*</span></label>
                        <select name="gender" class="form-control" required>
                            <option value="Male" <?php selected( ( $staff && isset( $staff->gender ) ) ? $staff->gender : '', 'Male' ); ?>><?php esc_html_e( 'Male', 'educore' ); ?></option>
                            <option value="Female" <?php selected( ( $staff && isset( $staff->gender ) ) ? $staff->gender : '', 'Female' ); ?>><?php esc_html_e( 'Female', 'educore' ); ?></option>
                            <option value="Other" <?php selected( ( $staff && isset( $staff->gender ) ) ? $staff->gender : '', 'Other' ); ?>><?php esc_html_e( 'Other', 'educore' ); ?></option>
                        </select>
                    </div>
                    <div class="col-md-3 mb-3">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="text" name="whatsapp_no" class="form-control" placeholder="e.g., 01XXXXXXXXX" value="<?php echo ( $staff && isset( $staff->whatsapp_no ) ) ? esc_attr( $staff->whatsapp_no ) : ''; ?>">
                    </div>
                </div>
            ...
            <div class="educore-step-content" id="educore-step-2">
                ...
                <div class="row">
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="text" name="designation" class="form-control" placeholder="e.g., Assistant Teacher, Lecturer" value="<?php echo ( $staff && isset( $staff->designation ) ) ? esc_attr( $staff->designation ) : ''; ?>" required>
                    </div>
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="staff_type" class="form-control" required>
                            ...
                            <option value="Teacher (School)" <?php selected( ( $staff && isset( $staff->staff_type ) ) ? $staff->staff_type : '', 'Teacher (School)' ); ?>><?php esc_html_e( 'Teacher (School)', 'educore' ); ?></option>
                            <option value="Teacher (College)" <?php selected( ( $staff && isset( $staff->staff_type ) ) ? $staff->staff_type : '', 'Teacher (College)' ); ?>><?php esc_html_e( 'Teacher (College)', 'educore' ); ?></option>
                            <option value="Officer" <?php selected( ( $staff && isset( $staff->staff_type ) ) ? $staff->staff_type : '', 'Officer' ); ?>><?php esc_html_e( 'Officer', 'educore' ); ?></option>
                            <option value="Staff" <?php selected( ( $staff && isset( $staff->staff_type ) ) ? $staff->staff_type : '', 'Staff' ); ?>><?php esc_html_e( 'Staff', 'educore' ); ?></option>
                        </select>
                    </div>
                    <div class="col-md-3 mb-3">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="text" name="index_no" class="form-control" placeholder="e.g., T1029384" value="<?php echo ( $staff && isset( $staff->index_no ) ) ? esc_attr( $staff->index_no ) : ''; ?>">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="subject_expert" class="form-control" placeholder="e.g., Mathematics, English" value="<?php echo ( $staff && isset( $staff->subject_expert ) ) ? esc_attr( $staff->subject_expert ) : ''; ?>">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Highest Qualification', 'educore' ); ?></label>
                        <input type="text" name="highest_degree" class="form-control" placeholder="e.g., MA in English, B.Sc" value="<?php echo ( $staff && isset( $staff->highest_degree ) ) ? esc_attr( $staff->highest_degree ) : ''; ?>">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Quota Category', 'educore' ); ?></label>
                        <select name="quota_type" class="form-control">
                            <option value="General" <?php selected( ( $staff && isset( $staff->quota_type ) ) ? $staff->quota_type : '', 'General' ); ?>><?php esc_html_e( 'General', 'educore' ); ?></option>
                            <option value="Freedom Fighter" <?php selected( ( $staff && isset( $staff->quota_type ) ) ? $staff->quota_type : '', 'Freedom Fighter' ); ?>><?php esc_html_e( 'Freedom Fighter', 'educore' ); ?></option>
                            <option value="Tribal" <?php selected( ( $staff && isset( $staff->quota_type ) ) ? $staff->quota_type : '', 'Tribal' ); ?>><?php esc_html_e( 'Tribal', 'educore' ); ?></option>
                            <option value="Other" <?php selected( ( $staff && isset( $staff->quota_type ) ) ? $staff->quota_type : '', 'Other' ); ?>><?php esc_html_e( 'Other', 'educore' ); ?></option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="joining_date" class="form-control" value="<?php echo ( $staff && isset( $staff->joining_date ) ) ? esc_attr( $staff->joining_date ) : esc_attr( date( 'Y-m-d' ) ); ?>" required>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Gross / Basic Salary (৳)', 'educore' ); ?> <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" name="salary" class="form-control" value="<?php echo ( $staff && isset( $staff->salary ) ) ? esc_attr( $staff->salary ) : '0.00'; ?>" required>
                    </div>
                </div>
            </div>
            ...
            <div class="educore-step-content" id="educore-step-3">
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="bank_name" class="form-control" placeholder="e.g., Sonali Bank PLC" value="<?php echo ( $staff && isset( $staff->bank_name ) ) ? esc_attr( $staff->bank_name ) : ''; ?>">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Bank Account Number', 'educore' ); ?></label>
                        <input type="text" name="bank_acc_no" class="form-control" placeholder="13-17 Digit" value="<?php echo ( $staff && isset( $staff->bank_acc_no ) ) ? esc_attr( $staff->bank_acc_no ) : ''; ?>">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Bank Routing Number', 'educore' ); ?></label>
                        <input type="text" name="bank_routing" class="form-control" placeholder="9 Digit Routing Code" value="<?php echo ( $staff && isset( $staff->bank_routing ) ) ? esc_attr( $staff->bank_routing ) : ''; ?>">
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="emergency_relation" class="form-control" placeholder="e.g., Spouse, Brother" value="<?php echo ( $staff && isset( $staff->emergency_relation ) ) ? esc_attr( $staff->emergency_relation ) : ''; ?>">
                    </div>
                    <div class="col-md-4 mb-3">
            ...
            <div class="educore-step-content" id="educore-step-4">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="status" class="form-control">
                            <option value="Active" <?php selected( ( $staff && isset( $staff->status ) ) ? $staff->status : '', 'Active' ); ?>><?php esc_html_e( 'Active', 'educore' ); ?></option>
                            <option value="Resigned" <?php selected( ( $staff && isset( $staff->status ) ) ? $staff->status : '', 'Resigned' ); ?>><?php esc_html_e( 'Resigned / Left', 'educore' ); ?></option>
                            <option value="Suspended" <?php selected( ( $staff && isset( $staff->status ) ) ? $staff->status : '', 'Suspended' ); ?>><?php esc_html_e( 'Suspended', 'educore' ); ?></option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Present Address', 'educore' ); ?></label>
                        <textarea name="address" class="form-control" rows="3" placeholder="Vill/Road, Post Office, Upazila, District"><?php echo ( $staff && isset( $staff->address ) ) ? esc_textarea( $staff->address ) : ''; ?></textarea>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold"><?php esc_html_e( 'Permanent Address', 'educore' ); ?></label>
                        <textarea name="permanent_address" class="form-control" rows="3" placeholder="Vill/Road, Post Office, Upazila, District"><?php echo ( $staff && isset( $staff->permanent_address ) ) ? esc_textarea( $staff->permanent_address ) : ''; ?></textarea>
                    </div>
                </div>
